...
static void dump_entries(int num) {
    ...
    BOOT_LOG_INF("Partition table with %d entries:", num);
    for (i = 0; i < num; i++) {
        ...
        BOOT_LOG_INF("%d: %s %x%x %d", i + 1, name,
...
static int load_mbr_header(uintptr_t image_handle, mbr_entry_t *mbr_entry) {
    ...
    if (result != 0) {
        BOOT_LOG_ERR("Failed to read data (%i)\n", result);
        return result;
    }
    ...
    if ((mbr_sector[LEGACY_PARTITION_BLOCK_SIZE - 2] != MBR_SIGNATURE_FIRST) ||
        ...
        BOOT_LOG_ERR("MBR signature isn't correct");
        return -ENOENT;
    }
...
static int load_gpt_header(uintptr_t image_handle) {
    ...
    if (header_crc != calc_crc) {
        BOOT_LOG_ERR("Invalid GPT Header CRC: Expected 0x%x but got 0x%x.\n",
                     header_crc, calc_crc);
        return -EINVAL;
    }
...
int load_partition_table(unsigned int image_id) {
    ...
    if (result != 0) {
        BOOT_LOG_WRN("Failed to obtain reference to image id=%u (%i)\n", image_id,
                    result);
        return result;
    }
    ...
    if (result != 0) {
        BOOT_LOG_WRN("Failed to open image id=%u (%i)\n", image_id, result);
        return result;
    }

    result = load_mbr_header(image_handle, &mbr_entry);
    if (result != 0) {
        BOOT_LOG_WRN("Loading mbr header failed with image id=%u (%i)\n", image_id,
                    result);
        return result;
    }
    if (mbr_entry.type == PARTITION_TYPE_GPT) {
        BOOT_LOG_INF("Loading gpt header");
        result = load_gpt_header(image_handle);
        assert(result == 0);
        if (result != 0) {
            BOOT_LOG_ERR("Failed load gpt header! %i", result);
            goto load_partition_table_exit;
        }
        result = io_seek(image_handle, IO_SEEK_SET, GPT_ENTRY_OFFSET);
        assert(result == 0);
        if (result != 0) {
            BOOT_LOG_ERR("Failed seek gpt header! %i", result);
            goto load_partition_table_exit;
        }
        result = verify_partition_gpt(image_handle);
        if (result != 0) {
            BOOT_LOG_ERR("Failed verify gpt partition %i", result);
            goto load_partition_table_exit;
        }
    } else {
